...
class prop_graph:
    ...
    def build_graph(self):
        ...
        for i in range(len(df)):
            ...
            if self.post_exist == False and self.source_post_content[0:10] in content:
    ...
    def plot_depth_time(self, separate_ratio:float=1):
        ...
        # 计算深度-时间信息
        depth_list = []
        self.d_t_list = list(range(int(self.start_timestamp), int(self.end_timestamp), 1))  # 按正常间隔为1算的 time list, depth-time的信息要足够详细
        depth = 0
    ...
    def plot_scale_time(self, separate_ratio:float=1.):
        ...
        """
        self.node_nums = []
        separate_point = int(int(self.start_timestamp) + separate_ratio*(int(self.end_timestamp)-int(self.start_timestamp)))  # start_time到separate point之间的数据详细刻画，separate point到end_time的数据粗略刻画

        self.s_t_list = list(range(int(self.start_timestamp), separate_point, 1))  #  + list(range(separate_point, int(self.end_time), 1000))
