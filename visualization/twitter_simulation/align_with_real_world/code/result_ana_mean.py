...
def plot_trend(db_folders:List, db_types:List):
    ...
    for stat_index, stat_name in enumerate(stats_name):
        ...
        for db_index, db_type in enumerate(db_types):
            ...
            ax.fill_between(range(stats[db_index][stat_index].shape[1]), mean_values - confidence_interval, mean_values + confidence_interval, 
                            color=colors[db_index], alpha=0.2, label=f'{db_type} 95% Confidence Interval')
