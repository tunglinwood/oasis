...
def gen_topics():
    ...
    expanded_combinations = expanded_combinations[:total]  # 截取前10000个
    # Step 3: 打乱顺序
...
topics = [
    "Politics", "Urban Legends", "Business", "Terrorism & War", "Science & Technology", "Entertainment",
    "Natural Disasters", "Health", "Education"
]
...
p_mbti = [0.12625, 0.11625, 0.02125, 0.03125, 0.05125, 0.07125, 0.04625, 0.04125, 0.04625, 0.06625, 0.07125, 0.03625,
          0.10125, 0.11125, 0.03125, 0.03125]
...
def create_user_profile(i):
    ...
    print(
        f"Person {i + 1}: Age={age}, MBTI={mbtis[mbti_index[i]]}, Gender={genders[gender_index[i]]}, Profession={professions[profession_index[i]]}")
    try:
        return generate_user_profile(age, mbtis[mbti_index[i]], genders[gender_index[i]],
                                     professions[profession_index[i]], [topics[x] for x in topic_index[i]])
...
with ThreadPoolExecutor(max_workers=50) as executor:
    ...
    for future in as_completed(futures):
        ...
        if len(user_dict) % 5000 == 0:
            ...
            with open(f"./large_3/{model}_{len(user_dict)}_agents.json", "w") as f:
