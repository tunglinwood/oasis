...
async def test_single_interview_action(setup_interview_test):
    ...
    try:
        ...
        profile = {
            ...
            "edges": [],
            "other_info": {
                "user_profile": "I am a test agent with strong opinions on technology.",
                "mbti": "INTJ",
        ...
        user_info = UserInfo(name=real_name, description=description, profile=profile)
        agent = SocialAgent(agent_id=0, user_info=user_info, twitter_channel=channel)
        
        # Sign up the agent
        return_message = await agent.env.action.sign_up("testuser", "TestUser", "A test bio.")
    ...
    try:
        ...
        for i in range(3):
            ...
            profile = {
                ...
                "edges": [],
                "other_info": {
                    "user_profile": f"I am test agent {i} with unique perspectives.",
                    "mbti": "INTJ",
                    "activity_level": ["online"] * 24,
                    "activity_level_frequency": [5] * 24,
                    "active_threshold": [0.5] * 24,
            ...
            user_info = UserInfo(name=real_name, description=description, profile=profile)
            agent = SocialAgent(agent_id=i, user_info=user_info, twitter_channel=channel)
            
            # Sign up the agent
            return_message = await agent.env.action.sign_up(f"testuser{i}", f"TestUser{i}", f"Test bio {i}.")
            assert return_message["success"] is True
            agents.append(agent)
...
async def test_interview_data_retrieval(setup_interview_test):
    ...
    try:
        ...
        for i in range(2):
            ...
            return_message = await agent.env.action.sign_up(f"testuser{i}", f"TestUser{i}", f"Bio {i}.")
            assert return_message["success"] is True
            agents.append(agent)
        ...
        interview_data = [
            ...
            (0, "What are your career goals?"),  # Second interview with agent 0
        ]
...
async def test_interview_error_handling(setup_interview_test):
    ...
    try:
        ...
        return_message = await agent.env.action.sign_up("testuser", "TestUser", "Test bio.")
        assert return_message["success"] is True
        agents.append(agent)

        # Test with empty prompt
        return_message = await agent.env.action.interview("")
        assert return_message["success"] is True  # Empty prompt should still work
